...
def assemble(
    ...
    try:
        module.keras_operation = keras.models.Model(
            inputs=[input], outputs=[output], name="-".join(module.ID.split())
        )
    except ValueError as e:
        print(
            indent
            + "    - Crashed with input: {} and output: {}".format(
...
def connect_operation_to_previous(
    node: Base, previous: list, input_layer: keras.layers.Input, indent
):
    ...
    if isinstance(node, Module):
        return assemble(
            node, in_shape=previous_output.shape, is_root=False, indent=indent + "    "
        )(previous_output)
    else:  # Operation
...
def get_tensors(previous, current):
    ...
    for prev in previous:
        if needs_flatten(prev):
            ...
            tensors += [flatten]
            print(
                "    - Flatten layer from {} to {}".format(
                    prev.keras_tensor.shape, flatten
                )
            )
        else:
